...
public class spawnPlayer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(PlayerPrefs.GetInt("Player1") == 0){
            player1 = Instantiate(blue, p1, Quaternion.identity);
        }
        else{
            player1 = Instantiate(red, p1, Quaternion.identity);
        }

        if(PlayerPrefs.GetInt("Player2") == 0){
            player2 = Instantiate(blue, p2, Quaternion.identity);
        }
        else{
            player2 = Instantiate(red, p2, Quaternion.identity);
        }
